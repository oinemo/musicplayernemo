...
let songs = [
    {
        ...
        artist: "Radiohead",
        album: "OK Computer",
        albumArtUrl: "https://upload.wikimedia.org/wikipedia/id/2/24/Radiohead_-_OK_Computer.png?20221028020255",
        audioSrc: "audio/letdownlagi.mp3",
        videoBgSrc: "videos/letdown.mp4", // Path video background khusus lagu ini
        // Lirik dengan timestamp dalam detik
        lyrics: [
            { time: 0.3, text: "Let down and hang..." },
            { time: 9.5, text: "Let down and hang..." },
            { time: 24.9, text: "You know, you know where you are with" },
            { time: 32.2, text: "You know where you are with" },
            { time: 36.6, text: "Floor collapsing" },
            { time: 38.7, text: "Floating"},
            { time: 41.8, text: "Bouncing back and"},
            { time: 43.7, text: "One day..."},
            { time: 46.2, text: "I'm gonna grow wings"},
            { time: 50.7, text: "A chemical reaction"},
            { time: 55.2, text: "Hysterical and useless"},
            { time: 59.6, text: "Hysterical and.."},
            { time: 62.5, text: "Let down and hanging around"},
            { time: 69.4, text: "Crushed like a bug in the ground"},
            { time: 76.3, text: "Let down and hanging around..."},
        ]
    },
    {
        id: 2,
...
function showPlayerPage() {
    ...
    if (currentSong && currentSong.videoBgSrc) {
        ...
        backgroundVideo.load();
        // Loop hanya jika bukan lagu "Let Down"
     if (currentSong.title === "Let Down") {
        backgroundVideo.loop = false;
      } else {
        backgroundVideo.loop = true;
      }

// Reset opacity setiap buka lagu
backgroundVideo.style.opacity = "1";

// Play video
backgroundVideo.play().catch(e => console.error("Error playing video background:", e));

// Fade out jika Let Down selesai
backgroundVideo.onended = () => {
  if (currentSong.title === "Let Down") {
    backgroundVideo.style.transition = "opacity 2s ease";
